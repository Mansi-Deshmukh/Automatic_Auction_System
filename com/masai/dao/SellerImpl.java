...
public class SellerImpl implements SellerDao  {
	...
	@Override
	public String updateItem(String product,int price,int quantity,int itemId) throws SellerException {
	    String message="Not updated yet......";

	    try(Connection conn=DBUtil.provideConnection()){
	    	PreparedStatement ps=conn.prepareStatement("update sellItem set product=?, price=?, quantity=?  where itemId=?");
	...
	public void updateQuantity(int itemid) throws SellerException {
		...
		try(Connection conn=DBUtil.provideConnection()){
	    	PreparedStatement ps=conn.prepareStatement("update sellitem set quantity=quantity-1  where itemId=?");
